...
class Manager(object):
    ...
    def _createBufWindow(self):
        if self._winPos == 0:
            ...
            vim.command("silent! noa keepj bo %d sp %s" % (self._winHeight, self._bufName))
        elif self._winPos == 2:
            vim.command("silent! noa keepj to %d sp %s" % (self._winHeight, self._bufName))
        else:
            vim.command("silent! noa keepj to %d vsp %s" % (self._winHeight, self._bufName))
